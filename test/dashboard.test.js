/**
 * Banco de pruebas del cálculo del dashboard.
 *
 *   node test/dashboard.test.js
 *
 * Importa js/calculo.js de verdad. Antes había que recortar el <script> de
 * index.html con una expresión regular y montarle un DOM de mentira: ese apaño
 * era justo el síntoma de que el código no se podía importar. Ahora el cálculo
 * es puro —entran filas, salen números— y se prueba sin navegador.
...
(async () => {
  // js/ es un módulo ES (lo dice su package.json), así que se importa con import().
  const { agregados, analisisEstaciones, ordenarTickets, ejeTickets } =
    await import('../js/calculo.js');
  ...
  prueba('El modo lo elige quien llama: el cálculo no guarda estado', () => {
    const filas = [
      reg({ id: 'A', tipo: 'GLP', kmGLP: 400, kmCalculo: 400, consumoReal: 10, consumoCoche: 8, costeKmReal: 0.09, costeKmCoche: 0.072 })
    ];
    cerca(agregados(filas, 'real').consumoGLP, 10);
    cerca(agregados(filas, 'coche').consumoGLP, 8);
  });
  ...
  prueba('Dos repostajes del mismo combustible el mismo día ya no se pisan', () => {
    ...
    const tickets = ordenarTickets(filas);
  ...
  prueba('La ventana de comparación se puede ajustar desde fuera', () => {
    const filas = [
      reg({ id: '1', fecha: '2026-01-10', tipo: 'GLP', precio: 0.80, litros: 40, estacion: 'A' }),
      reg({ id: '2', fecha: '2026-03-01', tipo: 'GLP', precio: 1.20, litros: 40, estacion: 'B' })
    ];
    assert.ok(analisisEstaciones(filas, 45).lista.every(x => x.desviacion === null));
    assert.ok(analisisEstaciones(filas, 120).lista.every(x => x.desviacion !== null));
  });
